...
Require Import mcextra prodvect mxpred hermitian.
...
Program Canonical ffun_add_monoid (I : finType) (G : zmodType) :=
  @Monoid.Law {ffun I -> G} 0 +%F _ _ _.

Next Obligation. by move=> I G; apply: ffun_addA. Qed.
Next Obligation. by move=> I G; apply: ffun_add0. Qed.
Next Obligation. by move=> I G x; rewrite ffun_addC ffun_add0. Qed.

Program Canonical ffun_add_comoid (I : finType) (G : zmodType) :=
  @Monoid.ComLaw {ffun I -> G} 0 (ffun_add_monoid I G) _.

Next Obligation. by move=> I G; apply: ffun_addC. Qed.

Program Canonical ffun_mul_monoid (I : finType) (R : ringType) :=
  @Monoid.Law {ffun I -> R} 1 *%F _ _ _.

Next Obligation. by move=> I G; apply: ffun_mulA. Qed.
Next Obligation. by move=> I G; apply: ffun_mul_1l. Qed.
Next Obligation. by move=> I G; apply: ffun_mul_1r. Qed.

Program Canonical ffun_mul_comoid (I : finType) (R : comRingType) :=
  @Monoid.ComLaw {ffun I -> R} 1 (ffun_mul_monoid I R) _.

Next Obligation. by move=> I G; apply: ffun_mulC. Qed.

Program Canonical ffun_muloid (I : finType) (R : ringType) :=
  @Monoid.MulLaw {ffun I -> R} 0 (ffun_mul_monoid I R) _ _.

Next Obligation.
by move=> I R f; apply/ffunP=> i; rewrite !ffunE mul0r.
Qed.

Next Obligation.
by move=> I R f; apply/ffunP=> i; rewrite !ffunE mulr0.
Qed.

Program Canonical ffun_addoid (I : finType) (R : comRingType) :=
  @Monoid.AddLaw {ffun I -> R} 0 *%F (ffun_add_comoid I R) _ _.

Next Obligation. by move=> I G; apply: ffun_mul_addl. Qed.
Next Obligation. by move=> I G; apply: ffun_mul_addr. Qed.
...
Notation "f .[ i0 <- v ]" := (@ffun_set _ _ i0 v f)
  (at level 2, i0 at level 200, v at level 200, format "f .[ i0  <-  v ]")
   : fun_scope.
...
Let B := mvector (fun i => 'I_(Vector.dim (E i))).
Let T := {ffun B -> k^o}.

Lemma dim_ffunr : Vector.dim [vectType k of T] = #|[finType of B]|.
Proof. by rewrite /Vector.dim /= /Vector.dim /= muln1. Qed. 

Definition i2n (i : B) : 'I_(Vector.dim [vectType k of T])
...
Proof. 
move/eqP; rewrite boolp.not_existsP; apply contra_not => P.
by apply/mvectorP=> i'; move:(P i'); rewrite Bool.not_false_iff_true =>/eqP ->.
Qed.
...
Definition mlinear {G} (f : mvector E -> G) :=
  @mxlinear G [rmorphism of idfun] f.
...
Lemma mlinear_sum
    {G} a (x : forall {i : I}, 'I_(a i) -> E i) (f : _ -> G)
  : mlinear f -> f (\mvector_(i : I) \sum_(j < a i) x j) =
      \sum_(b : mvector (fun i : I => 'I_(a i))) f (\mvector_(i : I) x (b i)).
Proof. by apply: mxlinear_sum. Qed.
...
Let B i := mvector (fun j : J i => 'I_(Vector.dim (@E i j))).
...
Let B := mvector (fun i : I => 'I_(Vector.dim (E i))).
Let T := {ffun B -> k^o}.
...
(* -------------------------------------------------------------------- *)
Section SesqLinear.
Context {I : finType} (E : I -> vectType C) (F : vectType C).
...
Definition lifts (x y : T) : F :=
  lift2 conjC [rmorphism of idfun] f x y.
...
Lemma alinear_hdotp y :
  mxlinear (G := [vectType C of C^o]) conjC (hdotp^~ y).
Proof.
move=> /= i a x1 x2; rewrite -conj_hdotp linear_hdotp /=.
by rewrite linearD /= rmorphM /= !conj_hdotp.
Qed.
...
Qed.
Local Canonical tdotp_linear x := Linear (@linear_tdotp x).
Definition tdotpr x := (@tdotp)^~ x.
Lemma linear_tdotpr x : linear_for (conjC \; *%R) (tdotpr x).
Proof.
by move=>a b c; rewrite /tdotpr /tdotp (xlifts_alinear 
  alinear_hdotp linear_hdotp) /GRing.scale.
Qed.
Local Canonical tdotpr_linear x := Linear (@linear_tdotpr x).
Local Canonical tdotp_rev := [revop (@tdotpr) of (@tdotp)].
Local Canonical tdotp_is_bilinear := [bilinear of (@tdotp)].
...
Definition fbasis_seq := 
    [seq fbasis (n2i i) | i: 'I_(Vector.dim [vectType C of T])].
Lemma fbasis_size : size fbasis_seq == Vector.dim [vectType C of T].
Proof. by rewrite size_map size_enum_ord. Qed.
Definition fbase := nosimpl (Tuple fbasis_size).
Lemma fbasis_seqE (i: 'I_(Vector.dim [vectType C of T])) :
...
Lemma fbasis_basis : basis_of {: [vectType C of T]} fbase.
Proof.
...
rewrite linearZl linearZr/= fbasis_ortho 1 ?eq_sym ?nji ?mulr0//.
...
(* by elim/tensorW: x => x; rewrite tdotpE ge0_hdotp.  *)
Qed.
...
(* by elim/tensorW: x => x; rewrite tdotpE hdotp_eq0. *)
Qed.
...
(* elim/tensorW: y; elim/tensorW: x => x y.
by rewrite !tdotpE conj_hdotp. *)
Qed.

(* Lemma linear_tdotp x : linear_for *%R (tdotp x).
Proof.
move=> /= a; elim/tensorW=> u; elim/tensorW=> v.
rewrite /tdotp xlifts_linear //;
  [by apply: alinear_hdotp | by apply: linear_hdotp].
Qed. *)

Definition tensor_hermitianMixin :=
  HermitianMixin ge0_tdotp tdotp_eq0 conj_tdotp linear_tdotp.
Canonical tensor_hermitianType := HermitianType tensor_hermitianMixin.
...
Lemma tensor_chs_is_chs : 
  forall i j, [< r2v (delta_mx 0 i) : T; r2v (delta_mx 0 j) : T>] = (i == j)%:R.
Proof.
move=> i j; rewrite -(n2iK i) -(n2iK j) (inj_eq). 
apply: i2n_inj. rewrite -!inject_base.
set x := (\mvector_ (k : I) r2v (delta_mx 0 (n2i i k))).
set y := (\mvector_ (k : I) r2v (delta_mx 0 (n2i j k))).
have ->: [< ⊗x; ⊗y >] = \prod_k [< x k; y k>]. by rewrite -tdotpE.
rewrite (eq_bigr (fun k=> (n2i i k == n2i j k)%:R)).
by move=>k _; rewrite !mvE chsE.
case eqij: (n2i i == n2i j).
by move/eqP: eqij=>->; rewrite big1 // => k; rewrite eqxx.
by move: (idx_neq eqij)=>P; destruct P; rewrite (bigD1 x0) //= H mul0r.
Qed.

Lemma tensor_chs_dim_proper : (1 <= (Vector.dim [vectType C of T]))%N.
Proof.
rewrite {1}/Vector.dim /= /Vector.dim /=.
rewrite muln1 card_mv card_dep_ffun foldrE big_map big_enum /=.
apply prodn_gt0=> i. move: (dim_proper (E i)).
by rewrite /Vector.dim /= card_ord.
Qed.

Definition tensor_chsMixin := ChsMixin tensor_chs_is_chs tensor_chs_dim_proper.
Canonical tensor_chsType := ChsType tensor_chsMixin.

End chsTensorProducts.

Section TensorSpaceDim.
Context {I : finType} (E : I -> chsType).

Lemma dim_tensor : (Vector.dim (tensor_chsType E) = \prod_i Vector.dim (E i))%N.
Proof.
rewrite {1}/Vector.dim /= /Vector.dim /=.
rewrite muln1 card_mv card_dep_ffun foldrE big_map big_enum /=.
...
Definition idx S := mvector (fun i : {i : I | i \in S } => 'I_(Vector.dim (E (val i)))).
Definition Hf S  := (fun i : {i : I | i \in S } => E (val i)).
Definition Hs S  := tensor_chsType (@Hf S).

Lemma eb_mv S i : eb i = ⊗ \mvector_(j : {i : I | i \in S }) eb ((n2i i : idx S) j).
Proof. by rewrite /eb inject_baseV /n2i. Qed.

(* instead of using ordinal number as index, following allows to use idx directly *)
Definition deltav S (i : idx S) := eb (i2n i).
Lemma tcbase S (i j : idx S) : [< deltav i ; deltav j >] = (i == j)%:R.
Proof. by rewrite /deltav cbase; rewrite inj_eq //; apply: i2n_inj. Qed.

Lemma normdv1 S (i : idx S) : `|deltav i| = 1.
Proof. by rewrite hnormE tcbase eqxx sqrtC1. Qed.

Lemma decdv S (u: Hs S) : u = \sum_i [< deltav i ; u >] *: deltav i.
Proof.
rewrite -intro_ebl =>i; rewrite dotp_sumr (bigD1 (n2i i)) //= big1.
move=> j /negPf nji; rewrite dotpZr /deltav cbase eq_sym.
by rewrite (can2_eq (@i2nK _ _ _) (@n2iK _ _ _)) nji mulr0.
by rewrite dotpZr /deltav !n2iK cbase eqxx mulr1 addr0.
Qed.

Lemma conjvdv S (i : idx S) : (deltav i)^*v = deltav i.
Proof. by rewrite /deltav conjveb. Qed.

Definition cdv S (i: idx S) (u: Hs S) := [< deltav i; u >].
Lemma cdvE S (i: idx S) (u: Hs S) : cdv i u = [< deltav i; u >].
Proof. by []. Qed.

Lemma cdv_is_linear S (i: idx S) : scalar (cdv i).
Proof. by move=>a u v; rewrite /cdv linearP. Qed.
Canonical cdv_linear S (i: idx S) := Linear (cdv_is_linear i).

Definition cdvdelta S (i j: idx S) : cdv i (deltav j) = (i == j)%:R.
Proof. by rewrite /cdv tcbase. Qed.

Definition cdvdeltaC S (i j: idx S) : cdv i (deltav j) = cdv j (deltav i).
Proof. by rewrite !cdvdelta eq_sym. Qed.

Lemma deccdv S (u: Hs S) : u = \sum_i cdv i u *: deltav i.
Proof. by rewrite /cdv -decdv. Qed.

Lemma conjvcdv S (u: Hs S) i : cdv i u^*v = (cdv i u)^*.
Proof. by rewrite /cdv conjv_dotr conjvdv conj_dotp. Qed.

Lemma adjcdv S T (f: 'Hom(Hs S, Hs T)) i j : 
  cdv i (f^A (deltav j)) = (cdv j (f (deltav i)))^*.
Proof. by rewrite /cdv adj_dotEV conj_dotp. Qed.

Lemma dot_cdv S (u v: Hs S) : [< u; v>] = \sum_i (cdv i u)^* * (cdv i v).
Proof. 
rewrite {1}(deccdv u) {1}(deccdv v).
rewrite dotp_suml; apply eq_bigr=> i _; rewrite dotp_sumr (bigD1 i) //= big1.
by move=>j /negPf nji; rewrite dotpZl dotpZr tcbase eq_sym nji !mulr0.
...
by move: (P (n2i t)); rewrite /cdv /deltav !n2iK.
Qed.

Lemma intro_dvl S (u v: Hs S) : 
...
Lemma intro_dvr S (u v: Hs S) : 
  (forall i, [< u ; deltav i >] = [< v ; deltav i >]) <-> u = v.
Proof.
split; [| move=>-> //]; rewrite -intro_dvl=> P t.
by apply (can_inj conjCK); rewrite !conj_dotp.
Qed.

Lemma dotmvT S (x y: mvector (@Hf S)) :
  [< ⊗ x ; ⊗ y >] = \prod_i [< x i; y i>].
...
Lemma casthsE S T (eq_ST : S = T) (v : Hs S) i:
  casths eq_ST v i = v (castidx (esym eq_ST) i).
Proof. by case: T / eq_ST i => //=. Qed.
...
Proof. by case: S' / eqS; rewrite casths_id castidx_id. Qed.

Lemma cdv_cast S S' (eqS : S = S') (i : idx S) v :
    cdv (castidx eqS i) v = cdv i (casths (esym eqS) v).
Proof. by case: S' / eqS v=>v; rewrite casths_id castidx_id. Qed.

Lemma cdv_castV S S' (eqS : S = S') (i : idx S') v :
    cdv i (casths eqS v) = cdv (castidx (esym eqS) i) v.
Proof. by case: S' / eqS i=>i; rewrite casths_id castidx_id. Qed.
...
Lemma castlfE S T S' T' (eqST : (S = S') * (T = T')) (f : 'Hom(Hs S, Hs T)) u :
  castlf eqST f u = casths eqST.2 (f (casths (esym eqST.1) u)).
Proof. 
by do [case: eqST; case: S' /; case: T' /] in f u *; rewrite !casths_id castlf_id.
Qed.
...
Proof. by move/(canLR (castlfK _ _)). Qed.
...
Lemma castlf_complf S T S' T' (eqST: S = T') (f: 'Hom(Hs S, Hs T)) 
  (g: 'Hom(Hs S', Hs T')) :
  castlf (eqST, erefl _) f \o g = f \o castlf (erefl _, esym eqST) g.
Proof.
by move: f g; case: T' / eqST => f g; rewrite !castlf_id.
Qed.

Lemma castlf_complfV S T S' T' (eqST: T' = S) (f: 'Hom(Hs S, Hs T)) 
  (g: 'Hom(Hs S', Hs T')) :
  f \o castlf (erefl _, eqST) g = castlf (esym eqST, erefl _) f \o g.
Proof.
by move: f g; case: S / eqST => f g; rewrite !castlf_id.
Qed.

Lemma castlf_complfG S1 T1 S2 S2' T2 (eqST: (S2 = S2') * (T2 = S1)) 
  (f: 'Hom(Hs S1, Hs T1)) (g: 'Hom(Hs S2, Hs T2)) : 
  (f \o (castlf eqST g) = castlf (eqST.1, erefl _) 
    (castlf (esym eqST.2, erefl _) f \o g))%VF.
Proof.
move: f g. case: eqST => eq1 eq2; case: S2' / eq1; case: S1 / eq2 => f g.
by rewrite !castlf_id.
Qed.
...
Lemma castlf_adj S T S' T' (eqST : (S = S') * (T = T')) (f: 'Hom(Hs S, Hs T)) : 
  (castlf eqST f)^A = castlf (eqST.2,eqST.1) f^A.
Proof.
by do [case: eqST; case: S' /; case: T' /] in f *; rewrite !castlf_id.
Qed.

Lemma castlf_tr S T S' T' (eqST : (S = S') * (T = T')) (f: 'Hom(Hs S, Hs T)) : 
  (castlf eqST f)^T = castlf (eqST.2,eqST.1) f^T.
Proof.
by do [case: eqST; case: S' /; case: T' /] in f *; rewrite !castlf_id.
Qed.
...
Lemma conjvV S (x : mvector (@Hf S)) :
  (⊗ x)^*v = ⊗ (\mvector_(i:{i : I | i \in S}) ((x i)^*v)%VF).
Proof.
rewrite /inject /conjv r2vK; apply f_equal; apply/matrixP=> i j.
by rewrite !mxE rmorph_prod; apply eq_bigr=>k _; rewrite mvE /conjmx r2vK mxE.
Qed.

Lemma lfunPD S T (f g : 'Hom(Hs S, Hs T)) : 
  (forall i, f (deltav i) = g (deltav i)) <-> f = g.
Proof.
split=> [P |-> //]; apply/lfunP=> u; rewrite (deccdv u) !linear_sum /=.
by apply eq_bigr=> i _; rewrite !linearZ /= (P i).
Qed.

Lemma default_scast (i : I) :
  Vector.dim (E i) = Vector.dim (Hs [set i]).
Proof. by rewrite dim_setten big_set1. Qed.

Definition inject_sv (i : I) (vi : E i) : Hs [set i] :=
  r2v (castmx (erefl _, (default_scast i)) (v2r vi)).

Lemma inject_sv_dotE (i : I) (u v : E i) :
  [<inject_sv u; inject_sv v>] = [<u ; v>].
Proof.
by rewrite dotp_mulmx /inject_sv !r2vK trmx_cast/= 
  conjmx_cast castmx_mul castmx_id dotp_mulmx.
Qed.

Definition extract_sv (i : I) (vi : Hs [set i]) : E i :=
  r2v (castmx (erefl _, esym (default_scast i)) (v2r vi)).

Lemma inject_svK (i : I) : cancel (@inject_sv i) (@extract_sv i).
Proof. by move=>x; rewrite /extract_sv /inject_sv r2vK castmx_comp castmx_id v2rK. Qed.

Lemma extract_svK (i : I) : cancel (@extract_sv i) (@inject_sv i).
Proof. by move=>x; rewrite /extract_sv /inject_sv r2vK castmx_comp castmx_id v2rK. Qed.

Lemma inject_sv_inj (i : I) : injective (@inject_sv i).
Proof. exact: (can_inj (@inject_svK i)). Qed.

Lemma inject_sv_is_linear (i : I) : linear (@inject_sv i).
Proof. by move=>a x y; rewrite /inject_sv !linearP/=. Qed.
Canonical inject_sv_linear (i : I) := Linear (@inject_sv_is_linear i).

Lemma extract_sv_inj (i : I) : injective (@extract_sv i).
Proof. exact: (can_inj (@extract_svK i)). Qed.

Lemma extract_sv_is_linear (i : I) : linear (@extract_sv i).
Proof. by move=>a x y; rewrite /extract_sv !linearP/=. Qed.
Canonical extract_sv_linear (i : I) := Linear (@extract_sv_is_linear i).

Lemma extract_sv_dotE (i : I) (u v : Hs [set i]) :
  [<extract_sv u; extract_sv v>] = [<u ; v>].
Proof. by rewrite -inject_sv_dotE !extract_svK. Qed.

Definition inject_sf (i : I) (fi : 'End(E i)) : 'End(Hs [set i]) :=
  Vector.Hom (castmx ((default_scast i), (default_scast i)) (f2mx fi)).

Lemma inject_sf_comp (i : I) (fi gi : 'End(E i)) : 
  inject_sf fi \o inject_sf gi = inject_sf (fi \o gi).
Proof. by apply/val_inj=>/=; rewrite !f2mx_comp /inject_sf/= castmx_mul. Qed.

Lemma inject_sf_apply (i : I) (fi : 'End(E i)) (vi : E i) :
  inject_sf fi (inject_sv vi) = inject_sv (fi vi).
Proof. by rewrite unlock/= /inject_sv !r2vK castmx_mul. Qed.

Definition extract_sf (i : I) (fi : 'End(Hs [set i])) : 'End(E i) :=
  Vector.Hom (castmx (esym (default_scast i), esym (default_scast i)) (f2mx fi)).

Lemma inject_sfK (i : I) : cancel (@inject_sf i) (@extract_sf i).
Proof. by move=>x; rewrite /extract_sf /inject_sf/= castmx_comp castmx_id f2mxK. Qed.

Lemma extract_sfK (i : I) : cancel (@extract_sf i) (@inject_sf i).
Proof. by move=>x; rewrite /extract_sf /inject_sf/= castmx_comp castmx_id f2mxK. Qed.

Lemma inject_sf_inj (i : I) : injective (@inject_sf i).
Proof. exact: (can_inj (@inject_sfK i)). Qed.

Lemma inject_sf_is_linear (i : I) : linear (@inject_sf i).
Proof. by move=>a x y; rewrite /inject_sf !linearP/=. Qed.
Canonical inject_sf_linear (i : I) := Linear (@inject_sf_is_linear i).

Lemma extract_sf_inj (i : I) : injective (@extract_sf i).
Proof. exact: (can_inj (@extract_sfK i)). Qed.

Lemma extract_sf_is_linear (i : I) : linear (@extract_sf i).
Proof. by move=>a x y; rewrite /extract_sf !linearP/=. Qed.
Canonical extract_sf_linear (i : I) := Linear (@extract_sf_is_linear i).

Lemma extract_sf_comp (i : I) (fi gi : 'End(Hs [set i])) : 
  extract_sf fi \o extract_sf gi = extract_sf (fi \o gi).
Proof. by apply/val_inj=>/=; rewrite !f2mx_comp /extract_sf/= castmx_mul. Qed.

Lemma extract_sf_apply (i : I) (fi : 'End(Hs [set i])) (vi : Hs [set i]) :
  extract_sf fi (extract_sv vi) = extract_sv (fi vi).
Proof. by rewrite unlock/= /extract_sv !r2vK castmx_mul. Qed.
...
(* tensor product of fset, for define \tensor_(i : J) E i later *)
Section FsetTensorVect.
Context {I : finType} (E : I -> chsType).
Variables (J : finType) (fs: J -> {set I}).

(* a finType index J *)
(* fs : map each index : J to a {fset I} *)
...
Definition flatidx (i: 'Idx[E]_bdom) : {dffun forall i : J, 'Idx[E]_(fs i)} 
  := [ffun s : J => \mvector_(j : {i | i \in fs s}) i (incl (map_cover s) j)]. 
...
have Q: val k = val j. by []. symmetry; move: (preidxP k).
apply boolp.contraPP; set m:= preidx k; rewrite Q.
move/eqP=> P1; move: (@dis _ _ P1). rewrite disjoint_subset=>/subsetP P2.
by apply/negP; apply: (P2 (val j)); destruct j.
Qed.

Definition packidx (f: {dffun forall i: J, 'Idx[E]_(fs i)}) : 'Idx[E]_bdom
  := \mvector_(j : {i | i \in bdom}) f (preidx j) (setsub (preidxP j)).

Lemma packidxE (f : {dffun forall i : J, 'Idx[E]_(fs i)}) : disf -> 
  forall (i : J) (j: {k | k \in fs i}), packidx f (incl (map_cover i) j) = f i j.
Proof. 
move=>dis i j; rewrite mvE. 
...
Definition injectv (x: mvector (fun i: J => 'H[E]_(fs i))) : 'H[E]_bdom 
...
Proof.
rewrite /injectv (bigD1 b)//=.
rewrite (eq_bigr (fun=>1)).
by move=>i _; rewrite mvE cdvdelta eqxx.
...
rewrite (eq_bigr fk)=>[k _|]; first by rewrite mvE cdvdelta.
...
Proof.
move=> dis; rewrite /hdotp /injectv dotp_suml.
...
by rewrite dotpZl dotpZr tcbase eq_sym niv !mulr0.
...
by move=>i _; rewrite dotpZl dotpZr tcbase eqxx mulr1 rmorph_prod big_split.
...
Proof.
rewrite /injectv scaler_sumr; apply eq_bigr=>e _; rewrite scalerA.
...
Proof.
rewrite /injectv -big_split /=; apply eq_bigr=>e _. rewrite -scalerDl.
congr (_ *: _); rewrite !(bigD1 i (P := predT)) //=.
rewrite !(msetii, mvE) linearD /= mulrDl; congr (_ * _ + _ * _);
 by apply/eq_bigr=> j ne_ji; rewrite !(mset_ne, mvE) 1?eq_sym // addr0.
Qed.

Lemma injectv_is_mlinear : mlinear injectv.
Proof.
...
- by apply: mlinear0 => //; apply/injectv_is_mlinear.
apply/'exists_eqP; apply/contra_eqT: eq0 => eq0.
have {}eq0: forall i, exists j, cdv j (x i) != 0.
- move=> i; apply/existsP; rewrite -negb_forall.
  apply/contra: eq0 => /forallP /= eq0; apply/existsP.
  exists i. apply/eqP. rewrite -intro_dvl => j.
  by rewrite linear0; apply/eqP.
pose e := [ffun i : J => xchoose (eq0 i)].
rewrite /injectv; apply/eqP=>/cdvP /(_ (packidx e)) => /eqP; apply/negP.
rewrite linear0 linear_sum (bigD1 (packidx e)) //= [\sum_( _ | _) _]big1.
by move=>j /negPf nje; rewrite linearZ /= cdvdelta eq_sym nje mulr0.
rewrite linearZ /= cdvdelta eqxx packidxK // addr0 mulr1.
...
Section FsetTensorLfun.
Context {I : finType} (E : I -> chsType).

(* fs : J -> {fset I}, for domain of each component *)
(* fs' : J -> {fset I}, for codomain of each component *)
(* f (j : J) : the linear functions of each component, Hs (fs j) -> Hs (fs' j) *)
Variables (J: finType) (fs fs': J -> {set I}).
Variable f : forall i : J, 'F[E]_((fs i), (fs' i)).
...
Definition liftlf_r (x : 'H[E]_bdom) (b : 'Idx[E]_bdom) :=
  (cdv b x) *: injectv (applyf (\mvector_(i : J) deltav (flatidx b i))).

Definition liftlf_def (x : 'H[E]_bdom) : 'H[E]_bcdom :=
  \sum_(b : 'Idx[E]_bdom) liftlf_r x b.

Lemma liftlf_def_is_linear : linear liftlf_def.
Proof.
move=>a u v; rewrite /liftlf_def scaler_sumr -big_split /=.
by apply eq_bigr=>i _; rewrite /liftlf_r linearP scalerDl scalerA.
Qed.
Canonical liftlf_def_linear := Linear liftlf_def_is_linear.
Definition liftlf := linfun liftlf_def.

(* if domains are well-defined, i.e., pairwise disjoint *)
Lemma liftlfE : disf fs ->  forall x, liftlf (injectv x) = injectv (applyf x).
Proof.
move=>dis x; rewrite /liftlf lfunE /= /liftlf_def /liftlf_r /applyf /injectv.
...
rewrite addr0 linearZ /= cdvdelta eqxx mulr1 -big_split /=.
...
move=>i _; rewrite mvE {1}(deccdv (x i)) !linear_sum; apply eq_bigr=> j _.
by rewrite !linearZ. by rewrite big_distr_idx.
Qed.

Lemma liftlf_delta b : liftlf (deltav b) = injectv (applyf (\mvector_(i : J) deltav (flatidx b i))).
Proof.
rewrite lfunE/=/liftlf_def/liftlf_r (bigD1 b)//= cdvdelta eqxx scale1r.
rewrite (eq_bigr (fun=>0)) => [j/negPf nj|].
by rewrite cdvdelta nj scale0r.
